...
write_tableHTML(OCCHTML(dataMEAN), file = paste0(game, " - ", QUA, " - dataMEAN.html"))
write_tableHTML(OCCHTML(dataPERC), file = paste0(game, " - ", QUA, " - dataPERC.html"))
write_tableHTML(OCCHTML(dataECDF), file = paste0(game, " - ", QUA, " - dataECDF.html"))

for (GPU in listGPU) {
	if (file.exists(GPU))	{
	write_tableHTML(OCCHTML(dataMEAN[dataMEAN$GPU==GPU,]), file = paste0(GPU, "\\", gameF, " - ", GPU, " - ", QUA, " - dataMEAN.html"))
	write_tableHTML(OCCHTML(dataPERC[dataPERC$GPU==GPU,]), file = paste0(GPU, "\\", gameF, " - ", GPU, " - ", QUA, " - dataPERC.html"))
	write_tableHTML(OCCHTML(dataECDF[dataECDF$GPU==GPU,]), file = paste0(GPU, "\\", gameF, " - ", GPU, " - ", QUA," - dataECDF.html"))
	}
	}
}

if (showDisplay){
	write_tableHTML(OCCHTML(dispMEAN), file = paste0(gameF, " - ", QUA, " - dispMEAN.html"))
	write_tableHTML(OCCHTML(dispPERC), file = paste0(gameF, " - ", QUA, " - dispPERC.html"))
	write_tableHTML(OCCHTML(dispECDF), file = paste0(gameF, " - ", QUA, " - dispECDF.html"))
	
	for (GPU in listGPU) {
	if (file.exists(GPU))	{
		write_tableHTML(OCCHTML(dispMEAN[dispMEAN$GPU==GPU,]), file = paste0(GPU, "\\", gameF, " - ", GPU, " - ", QUA, " - dispMEAN.html"))
		write_tableHTML(OCCHTML(dispPERC[dispPERC$GPU==GPU,]), file = paste0(GPU, "\\", gameF, " - ", GPU, " - ", QUA, " - dispPERC.html"))
		write_tableHTML(OCCHTML(dispECDF[dispECDF$GPU==GPU,]), file = paste0(GPU, "\\", gameF, " - ", GPU, " - ", QUA, " - dispECDF.html"))
	}
	}
}
