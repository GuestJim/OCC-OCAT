...
statGRAPH	=	function(DATA, r = 2, quan = QUAN)	{
	out	=	c(mean(DATA), median(DATA), qqslope(DATA, quan = quan))
	for (i in c(0.1, 1, 99, 99.9)/100)	{
		temp		=	c(quantile(DATA, i))
		names(temp)	=	paste0(i * 100)
		out			=	append(out, temp)
	}
...
writeOCC(rendECDF)
writeOCC(rendSTAT)
writeOCC(compTAB(rendMEAN, rendPERC, rendECDF), typeName = "rendCOMP")

if	(textAPI)	{
	for	(API in listAPI)	{
	writeSUB(rendMEAN, API)
	writeSUB(rendPERC, API)
	writeSUB(rendECDF, API)
	writeSUB(rendSTAT, API)
	writeSUB(compTAB(rendMEAN, rendPERC, rendECDF), API, typeName = "rendCOMP")
	}	}

for (GPU in listGPU)	{	if	(file.exists(GPU))	{
	writeGPU(rendMEAN, GPU)
	writeGPU(rendPERC, GPU)
	writeGPU(rendECDF, GPU)
	writeGPU(rendSTAT, GPU)
	writeGPU(compTAB(rendMEAN, rendPERC, rendECDF), GPU, typeName = "rendCOMP")
	}	}
}
}
