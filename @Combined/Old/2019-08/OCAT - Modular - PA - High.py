...
if "Performance Analysis" in droppedGame:
	droppedGame = droppedGame.replace(" Performance Analysis", "")

# for place in range(len(droppedPath.split("\\"))):
	# if "OCAT Data" in droppedPath.split("\\")[place]:
		# droppedGame = droppedPath.split("\\")[place - 1]
...
if not os.path.exists(outputFull):
	with open(scriptFull, 'r') as fref, open(outputFull, 'w') as fout:
		for line in fref:
			fout.write(line.replace("!PATH!", RPath).replace("!GAME!", droppedGame).replace("!API!", listclean(APIs)).replace("!QUA!", QUAs[0]).replace("!TYPE!", "Max").replace("!LOC!", locStr).replace("!LOCSHO!", locshoStr))
		fout.close()
...
if not os.path.exists(droppedPath + "OCAT - Combined - Output.r"):
	shutil.copyfile(scriptPath + "\\OCAT - Combined - Output.r", droppedPath + "@Combined - Output - High.r")

# if not os.path.exists(droppedPath + "OCAT - Combined - Output - High.r"):
	# shutil.copyfile(scriptPath + "\\OCAT - Combined - Output - High.r", droppedPath + "@Combined - Output - Max.r")
