...
if not os.path.exists(outputFull):
	with open(scriptFull, 'r') as fref, open(outputFull, 'w') as fout:
		for line in fref:
			fout.write(line.replace("!PATH!", RPath).replace("!GAME!", droppedGame).replace("!GPU!", GPUs[0]).replace("!LONG!", out).replace("!QUA!", QUAs[0]).replace("!LOC!", locStr).replace("!LOCSHO!", locshoStr))
		fout.close()
...
if not os.path.exists(outputFull):
	with open(scriptFull, 'r') as fref, open(outputFull, 'w') as fout:
		for line in fref:
			fout.write(line.replace("!PATH!", RPath).replace("!GAME!", droppedGame).replace("!GPU!", GPUs[0]).replace("!API!", listclean(APIs)).replace("!QUA!", QUAs[0]).replace("!TYPE!", "GPU").replace("!LOC!", locStr).replace("!LOCSHO!", locshoStr))
		fout.close()
...
#	no longer necessary, the Combined - Output file is able to do it all
