...
results <- read_csv("@Combined - !QUA!.csv", col_types = "?????????????????ffff")
...
customSave = function(type="", device=ggdevice, width=8, height=9, dpi=DPI) {
	if (exists("recording")) {	
		if (device=="png") {
		...
		if (device=="png") {
			ggsave(filename=paste0(gameF, " - ", type, ".png"), device=device, width=width, height=height, dpi=dpi)
		}	else if (device=="pdf")	{
			ggsave(filename=paste0(gameF, " - ", type, ".pdf"), device=device, width=width, height=height)
		}
	}
}

#Averages
ggplot(data = results) + ggtitle(paste0(game, " - ", QUA), subtitle = "Averages, Medians, and Percentiles") + 
...
facet_grid(rows = vars(API), cols = vars(Location), switch = "y") + 
scale_x_discrete(labels = labelBreak) + 
scale_y_continuous(name="Display Time (ms)", breaks=c(0, round(1000/ytimes, 2)), limits=c(NA,66.67), expand=c(0.02, 0), sec.axis = dup_axis()) + 
guides(fill = guide_legend(nrow = 1)) + theme(legend.position = "bottom")

customSave("Averages Disp")
}
...
ggtitle(paste0(game, " - ", QUA), subtitle = "MsBetweenPresent") + 
geom_hline(yintercept = 1000/60, color = "red") + 
...
ggtitle(paste0(game, " - ", QUA), subtitle = "MsBetweenDisplayChange") + 
geom_hline(yintercept = 1000/60, color = "red") + 
geom_point(alpha = 0.05) + 
geom_smooth(method="gam", formula= y ~ s(x, bs = "cs")) + 
facet_grid(cols = vars(GPU), rows = vars(Location, API), switch = "y") + 
...
ggtitle(paste0(game, " - ", QUA), subtitle="MsBetweenPresent consecutive differences") + 
...
ggtitle(paste0(game, " - ", QUA), subtitle="MsBetweenDisplayChange consecutive differences") + 
geom_point(alpha = 0.1) + 
stat_density_2d(geom = "polygon", aes(fill = stat(nlevel), alpha = stat(nlevel)), show.legend = FALSE) + 	scale_fill_viridis_c() + 
...
ggtitle(paste0(game, " - ", QUA), subtitle="Frequency Plot of MsBetweenPresents") + 
...
customSave("Freq Frame")

#Frequency Plot - Display Time
if (showDisplay){
ggplot(results, aes(MsBetweenDisplayChange)) + 
ggtitle(paste0(game, " - ", QUA), subtitle="Frequency Plot of MsBetweenDisplayChange") + 
geom_vline(xintercept = 1000/60, color = "red") + 
geom_freqpoly(binwidth=0.03, size=0) + 
facet_grid(cols = vars(GPU), rows = vars(Location, API), switch = "y") + 
scale_x_continuous(name="Display Time (ms)", breaks=seq(from=0, to=FtimeLimit, by=1000/60), labels=labelRound, limits = c(NA, FtimeLimit), expand=c(0.02, 0), sec.axis = dup_axis()) + 
expand_limits(x=c(1000/60, 1000/30)) + 
scale_y_continuous(name="Count", expand=c(0.02, 0))

customSave("Freq Display")
}

#QQ Plot - Frame Time
ggplot(results, aes(sample=MsBetweenPresents)) + 
ggtitle(paste0(game, " - ", QUA), subtitle="QQ Plot of MsBetweenPresents") + 
geom_hline(yintercept = 1000/60, color = "red") + 
geom_point(stat="qq") + 
facet_grid(cols = vars(GPU), rows = vars(Location, API), switch = "y") + 
...
ggtitle(paste0(game, " - ", QUA), subtitle="QQ Plot of MsBetweenDisplayChange") + 
geom_hline(yintercept = 1000/60, color = "red") + 
geom_point(stat="qq") + 
facet_grid(cols = vars(GPU), rows = vars(Location, API), switch = "y") + 
scale_y_continuous(name="Display Time (ms)", breaks=c(0, round(1000/ytimes, 2)), labels=labelRound, limits=c(NA, FtimeLimit), expand=c(0.02, 0)) + 
scale_x_continuous(name="Percentile", breaks=qnorm(c(.001, .01, .5, .99, .999)), labels=c("0.1", "\n1", "50", "\n99", "99.9"), minor_breaks=NULL, expand=c(0.02, 0))
# scale_x_continuous(name="Percentile", breaks=qnorm(c(.001, .01, .5, .99, .999)), labels=c("0.1", "\n1", "50 (Median)", "\n99", "99.9"), minor_breaks=NULL, expand=c(0.02, 0))

customSave("QQ Display")
}