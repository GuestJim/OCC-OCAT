...
if(textOUT) {
#	check for if the text output should be made
sink(paste0("Output - ", fileName, ".txt"), split=TRUE)
#	creates the TXT file with the same fileName as the data file
writeLines(fileName)
if (exists("recording")) {
...
#	to make generating the graphs easier, made a custom version of the ggsave function
#		the format of the output file name is set, with only the graph type being a variable for use
#		though the device is set by the ggdevice variable set earlier, it is still possible to change it
#		the custom width and height values have defaults of 16 and 9, but other custom values can be given
#			for PNGs, width and height should be used to control aspect ratio and dpi for resolution
#	technically just customSave() will work, but it should be given a string as an argument identifying the graph type
