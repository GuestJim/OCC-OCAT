library(readr)
library(ggplot2)
setwd("E:/Users/Jim/My Documents/OCC/@Reviews/Vega Efficiency/Data/Killing Floor 2/")

pngname = "Combined Results"
setname = " - \nKilling Floor 2 (Stock)"
...
if (TRUE){
ggplot(na.rm = TRUE, data = ADRN) + ggtitle(paste("QQ Plot of Clock Speeds", setname, sep = "")) + 
...
if (TRUE){
ggplot(na.rm = TRUE) + ggtitle(paste("Clock Speed Through Course", setname, sep = ""), subtitle="MHz") +
...
if (TRUE){
ggplot(na.rm = TRUE) + ggtitle(paste("HBM Through Course", setname, sep = ""), subtitle="MHz") +
...
ggplot(na.rm = TRUE) + ggtitle(paste("Clock Speed vs Power (Averages)", setname, sep = ""), subtitle = "MHz vs W") + 
scale_fill_gradient2("Power (W)", low="blue", mid = "green", midpoint = 225,  high="red", limits = c(50, 400), breaks = c(75,150,225,300,375)) +  
...
#Clock vs Power
if (TRUE){
ggplot(data = ADRN, aes(x = GPUSCLK, y = GPUPWR, group = RUN, color = RUN, fill = RUN)) + ggtitle(paste("Clock vs Power", setname, sep = ""), subtitle = "MHz vs W") + 
...
if (TRUE){
scaleFF = 7.5
ggplot(na.rm = TRUE) + ggtitle(paste("Comparison of Fan Speed", setname, sep = ""), subtitle="Average RPM") +
